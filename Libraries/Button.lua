...
function Button.new(tbl)
    ...
    --Check to see if description has a font
    ...
    --Assign the update text and clicked functions
        obj.executable = tbl.clicked
        obj.updateText = tbl.updateText

    --Check to see if an audio is enabled
        obj.audio = {
            audio = tbl.audio or assetloader:getAudio("button")
        }
        obj.audio.audio:setPitch(cosmeticRandom:random(.9,1))
...
function Button:clicked(passThrough)
    ...
    if self.audio.audio then
        love.audio.play(self.audio.audio)
    end
end
