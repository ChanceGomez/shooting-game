...
function Player:draw()
    ...
    --draw ammo count
        ...
        local background_x,background_y = 4,296
        local width,height = 24,8
        local margin = 4
        for i = 1, game.Affector:trigger("Max Ammo",game.Player.gun.maxAmmo) do
            local color = {.4,.4,.4,.7}
            love.graphics.setColor(color)
            love.graphics.rectangle("fill",background_x,background_y-(i*(height+margin)),width,height)
        end

        --show current ammo
        local ammo_x,ammo_y = 4,296
        local width,height = 24,8
        local margin = 4
        for i, bullet in ipairs(game.Player.gun.ammo) do
            ...
            love.graphics.setColor(color)
            love.graphics.rectangle("fill",ammo_x,ammo_y-(i*(height+margin)),width,height)
        end

    --Draw explosion count
        local explosion_x,explosion_y = 36,window.GameHeight
        local margin = 2
        local image = assetloader:getImage("bomb_ammo")
        
        for i = 1, self.grenades do
            local bombX,bombY = explosion_x, explosion_y - (i*(image:getWidth()+margin))
